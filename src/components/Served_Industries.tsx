import React from 'react';
import { motion } from 'framer-motion';
import { useInView } from 'react-intersection-observer';



interface Project {
    id: number;
    category: string;
    image: string;
    description: string[];
    link: string;

}

const listItemVariants = {
    hidden: { opacity: 0, x: 50 },
    visible: { opacity: 1, x: 0, transition: { duration: 0.4 } },
};

const listVariants = {
    hidden: {},
    visible: {
        transition: {
            staggerChildren: 0.15, // items animate one after another
        },
    },
};



const projects: Project[] = [
    {
        id: 1,
        category: "ED-TECH",
        image: "https://static.vecteezy.com/system/resources/previews/009/535/653/non_2x/e-learning-concept-hexagonal-with-education-icons-such-as-dna-earth-wifi-and-flask-bottle-on-blue-technology-background-vector.jpg",
        description: ["Interactive Websites & Learning Apps for EdTech Brands", "Digital Marketing to Boost Student Enrollments", "High-Impact Educational Videos & Visual Content", "Smart Tech Tools for Modern Learning"],
        link: "#",

    },
    {
        id: 2,
        category: "Healthcare",
        image: "data:image/jpeg;base64,",
        description: ["Custom Websites & Apps for Hospitals and Clinics", "Digital Marketing That Attracts More Patients", "Medical Videos & Visuals That Build Trust", "Smart Tools to Streamline Your Practice"],
        link: "#",

    },
    {
        id: 3,
        category: "Restaurant",
        image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8bb695_Lr8Bu2ifOwupEwWIykMnfBXtMnTw&s",
        description: ["Deliciously Designed Websites & Food Ordering Apps", "Digital Marketing That Fills Tables Fast", "Mouth-Watering Videos & Visual Content", "Smart Tools to Manage Orders & Customers"],
        link: "#",

    },
    {
        id: 4,
        category: "Real Estate Industry",
        image: "https://d17pa1qnt5ppyh.cloudfront.net/wp-content/uploads/2019/05/real-estate-industry-careers.jpg",
        description: ["Modern Websites & Apps to Showcase Properties", "Digital Marketing That Generates Property Leads", "Visuals That Sell Spaces Before the Visit", "Smart CRM & Booking Tools for Agents"],
        link: "#",

    },
    {
        id: 5,
        category: "Fitness and Wellness Industry",
        image: "https://i0.wp.com/hrnxt.com/wp-content/uploads/2019/11/pexels-photo-1954524.jpeg?fit=752%2C501&ssl=1",
        description: ["Branded Websites & Workout Apps That Convert", "Online Presence That Builds Your Fitness Tribe", "High-Energy Videos & Social Content", "Smart Tools for Bookings, Plans & Progress"],
        link: "#",

    },
    {
        id: 6,
        category: "E-commerce & D2C Brands",
        image: "https://indian-retailer.s3.ap-south-1.amazonaws.com/s3fs-public/2025-01/The%20E-Commerce%20Landscape%20in%202025%20Trends%2C%20Growth%2C%20and%20the%20Role%20of%20D2C%20Brands.jpg",
        description: ["Custom Online Stores That Drive Sales", "Marketing That Gets You More Orders", "Product Photos, Videos & Scroll-Stopping Creatives", "Smart Integrations for Smooth Operations"],
        link: "#",

    }
];

const ServedIndustries: React.FC = () => {
    const [ref, inView] = useInView({
        triggerOnce: true,
        threshold: 0.1,
    });



    return (
        <section id="ServedIndustries" className="py-20 relative">
            <div className="container mx-auto px-4 md:px-6">
                <motion.div
                    ref={ref}
                    initial={{ opacity: 0, y: 20 }}
                    animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                    transition={{ duration: 0.6 }}
                    className="text-center mb-16"
                >
                    <h2 className="text-3xl md:text-5xl font-display font-bold mb-4">
                        Industries we <span className="heading-gradient">Served</span>
                    </h2>
                    <p className="text-gray-300 max-w-2xl mx-auto">
                        Delivering experties across a range of Industries
                    </p>
                </motion.div>



                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {projects.map((project, index) => (
                        <motion.div
                            key={project.id}
                            initial={{ opacity: 0, y: 30 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.6, delay: index * 0.1 }}
                            className="glass-card overflow-hidden group cursor-pointer"
                            variants={listVariants}
                            whileHover="visible"
                        >
                            <div className="relative h-60 overflow-hidden group">
                                <img
                                    loading="lazy"
                                    src={project.image}
                                    alt="."
                                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                                />

                                <div className="absolute inset-0 bg-dark-300/60  group-hover:bg-black/80 transition-all duration-500 ease-in-out flex items-center justify-center p-4">
                                    <div className="relative w-full flex justify-center items-center">

                                        {/* Centered h2, fades out on hover */}
                                        <h2 className="absolute text-white drop-shadow-[0_2px_4px_rgba(255,105,180,0.3)] text-3xl font-bold transition-all duration-500 ease-in-out opacity-100 group-hover:opacity-0">
                                            {project.category}
                                        </h2>

                                        {/* Centered ul, fades in on hover */}
                                        <motion.ul
                                            className="absolute opacity-0 px-5 group-hover:opacity-100 transition-all duration-500 ease-in-out text-white space-y-1 text-center"
                                            // trigger animation on hover
                                        >
                                            {project.description.map((desc, index) => (
                                                <motion.li
                                                    key={index}
                                                    className="text-left list-disc"
                                                    variants={listItemVariants}
                                                >
                                                    {desc}
                                                </motion.li>
                                            ))}
                                        </motion.ul>

                                    </div>
                                </div>
                            </div>


                            {/* <div className="p-6">
                                <h3 className="text-xl font-semibold mb-2">{project.title}</h3>
                                <p className="text-gray-300 text-sm mb-4">{project.description}</p>
                                <div className="flex justify-between items-center">
                                    <span className="text-primary-400 text-sm font-medium">{project.category}</span>
                                    <a href={project.link} className="text-white hover:text-primary-400 transition-colors duration-300">
                                        <ExternalLink size={18} />
                                    </a>
                                </div>
                            </div> */}
                        </motion.div>
                    ))}
                </div>
            </div>



        </section >
    );
};

export default ServedIndustries;